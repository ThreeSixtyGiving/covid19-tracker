...
from ..settings import THREESIXTY_COLOURS, MAPBOX_TOKEN, SOURCES

def sources(s, without_geo=None):
    if not without_geo:
        without_geo = s.isnull().sum()

    return html.Header(className="media-card__byline", style={'textAlign': 'left'}, children=[
        html.H4(className='',
                children='Source for location (number of grants)'),
...
def geomap(grants):
    ...
    return html.Div(
        ...
        children=[
            html.Div(className="media-card__content", children=[
                dcc.Graph(
                    id='geo-map',
                    figure={
                        'data': [dict(
