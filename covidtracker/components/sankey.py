...
def sankey(grants, all_grants, funder_id, funder_name):
    ...
    nodes = [
        funder_name,
        name_for_other_recipients
    ]
    links = []

    # find grants where the funder is a recipient
    funder_is_recipient = all_grants[
        all_grants['recipientOrganization.0.id'] == funder_id
    ].groupby('fundingOrganization.0.name').sum()['amountAwarded']
    ...
    recipient_is_funder = all_grants[
        (all_grants['fundingOrganization.0.id'] == funder_id) &
        all_grants['_recipient_is_grantmaker']
    ].groupby('recipientOrganization.0.name').sum()['amountAwarded']
