...
def horizontal_bar(categories, value="count", text=None, 
    log_axis=False, colour='#237756', text_colour='#fff', 
    **kwargs):
    ...
    for k, x in enumerate(categories):
        hb_plot.add_trace(dict(
            ...
            textposition='auto' if not log_axis or not max_value or (
                (x[value] / max_value) > 0.05) else 'outside',
            textfont=dict(
                family='"Roboto", sans-serif',
                color=text_colour,
            ),
